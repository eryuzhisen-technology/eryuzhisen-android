...
//        return (Class<T>) types[0];
    ...
    public static <T> T newPresenter(Object obj) {
        if (!NaContract.View.class.isInstance(obj)) {
            throw new RuntimeException("no implement XContract.BaseView");
        }
        try {
            Class<?> currentClass = obj.getClass();
//            Class<?> viewClass = getViewInterface(currentClass);
            Class<?> presenterClass = getGenericClass(currentClass, NaContract.Presenter.class);
            Class<?> modelClass = getGenericClass(presenterClass, NaContract.Model.class);
//            Constructor construct = presenterClass.getConstructor(viewClass, modelClass);
            NaBasePresenter<?, ?> xBasePresenter = (NaBasePresenter<?, ?>) presenterClass.newInstance();
